...
def circular_padding(batch, kernel_size=3):
    padding_top = (kernel_size - 1) // 2
    padding_bottom = (kernel_size - 1) // 2
    padding_left = (kernel_size - 1) // 2
    padding_right = (kernel_size - 1) // 2

    right_side = batch[:, :, -padding_left:, :]
    left_side = batch[:, :, :padding_right, :]

    batch = tf.concat([right_side, batch, left_side], axis=2)
    batch = tf.pad(batch, [[[[0, 0], [padding_top, padding_bottom], [0, 0], [0, 0]]]])

    return batch


def unet_circular(input_size, n_filters=64):

    # Contracting Path (encoding)
    inputs = tf.keras.layers.Input(input_size)

    conv1 = circular_padding(inputs)
    conv1 = tf.keras.layers.Conv2D(n_filters,
                                   3,
                                   activation='relu',
                                   padding='valid',
